...
const CourseDetails = (props) =>{
	console.log(props.response.rating);
	return (
		<>
            ...
            <div className="tutor-col-9 tutor-col-md-100">
					...
					<div className="tutor-leadinfo-top-meta">
						<span className="tutor-single-course-rating">
							<div className="tutor-star-rating-group">
								{props.response.rating ? <RatingCom items={props.response.rating} /> : null}
								<div className="tutor-rating-gen-input">
									<input type="hidden" name="tutor_rating_gen_input" values={props.response.ratingDetails ? props.response.ratingDetails.ratingVal : null} /> 
								</div>
							</div>            
							<span className="tutor-single-rating-count">
								{props.response.ratingDetails ? props.response.ratingDetails[0].ratingVal : null}<i>({props.response.ratingDetails ? props.response.ratingDetails[0].ratingCount : null})</i>            
							</span>
						</span>
					</div>
					<h1 className="tutor-course-header-h1">{props.response.topic ? props.response.topic : null} : {props.response.title ? props.response.title : null} </h1>
				</div>
							...
							<li className="tutor-single-course-author-meta">
								<div className="tutor-single-course-avatar">
									<a href="https://demos.bolvo.com/eduwp4/profile/dinesh"> 
										<span className="tutor-text-avatar">{props.response.authorShortName ? props.response.authorShortName : null}</span>
									</a>
								</div>
								<div className="tutor-single-course-author-name">
									<span>by</span>
									<a href="#">{props.response.authorName ? props.response.authorName : null}</a>
								</div>
							</li>
							<li className="tutor-course-level">
								<strong>Course level:</strong>
										{props.response.level ? props.response.level : null}
							</li>
					...
					<div className="tutor-single-course-meta tutor-lead-meta">
						<ul>
							<li>
								...
								<a href="#">{props.response.categories ? props.response.categories : null}</a>
							</li>
							<li>
								<span>Total Hour</span>
									<i className="fa fa-clock-o">&nbsp;{props.response.duration ? props.response.duration : null}</i>                 
							</li>
							<li>
								<span>Total Enrolled</span>
									<i className="fa fa-user">&nbsp; {props.response.subsCount ? props.response.subsCount : null} </i>            
							</li>
							<li>
									<span>Last Update</span>
									{props.response.modifiedAt ? props.response.modifiedAt : null} 
							</li>
						</ul>
					</div>
					<div className="tutor-course-summery">
							<h4 className="tutor-segment-title">About Course</h4>
							{props.response.courseDetails ? props.response.courseDetails[0].about : null}
					</div>
				</div>	            
				<div className="tutor-single-course-segment  tutor-course-content-wrap">
					...
					<div className="tutor-course-content-content">
						<p>{props.response.courseDetails ? props.response.courseDetails[0].desc : null}</p>						
					</div>
				</div>
				...
				<div className="tutor-single-course-segment  tutor-course-topics-wrap">
					...
					<TotpicList items={props.response.topicsList} click={props.click} />
				</div>
				: null}
        ...
        return(
                <>
                        <div className="tt-post post-238 post type-post status-publish format-standard has-post-thumbnail hentry category-uncategorized tag-books tag-coading tag-education tag-graphic-design tag-library" key={props.keyv}>
                                <a className="tt-post-img custom-hover" target="_blank" href={props.link} title={props.title}>
